...
public class RaidHelperClient(HttpClient httpClient)
{
    ...
    {
        var response = await httpClient.GetFromJsonAsync<EventsResponse>($"v4/servers/{guildId}/events");
    ...
    public Task CreateEventAsync(ulong guildId, ulong channelId, CreateEventRequest request) =>
        httpClient.PostAsJsonAsync($"v4/servers/{guildId}/channels/{channelId}/event", request);
}
